...
ChunkManager::ChunkManager(BlockTypesPtr block_types) : block_types(block_types)
{
    ...
    /* World Generation */
    {
        ...
        {
            for(int z = 0; x < RENDER_DISTANCE; z++)
            {
                glm::vec2 chunk_position{x, z};
                Chunk* current_chunk = new Chunk(chunk_position, block_types, chunk_indices);
                ...
                if(z > 0)
                {
                    ...
                    current_chunk->neighbour_chunks.south = current_chunk;
                }
                if(x > 0)
                {
                    west_chunk = chunks[x - 1][z];
                    current_chunk->neighbour_chunks.west = west_chunk;
                    current_chunk->neighbour_chunks.east = current_chunk;
                }
            }
        }
    }
