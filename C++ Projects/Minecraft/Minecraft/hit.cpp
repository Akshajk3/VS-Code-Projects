...
PlayerHitray::PlayerHitray(World* world) : world(world), vector{0, 0, 0}, position(0), block(position), distance(0u)
{

}

bool PlayerHitray::check(float distance, const glm::vec3& current_block, const glm::vec3& next_block)
{
...
bool PlayerHitray::step()
{
    ...
    if(abs_vector.z)
    {
        float x = (0.5f - lpos.z) / abs_vector.z * abs_vector.x + lpos.x;
        float y = (0.5f - lpos.z) / abs_vector.z * abs_vector.y + lpos.y;
        float z = 0.5f;
