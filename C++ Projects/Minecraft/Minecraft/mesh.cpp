...
Mesh::Mesh(size_t max_quads) : max_quads(max_quads), current_quad_count(0)
{
...
void Mesh::pushBlock(const BlockType& block_type, const glm::vec3 pos, const BatchInfo& batch_info)
{
    ...
    for(uint16_t i = 0; i < block_type.get_quad_number(); i++)
    {
        ...
        {
            Quad current_quad;
            memcpy(&current_quad, block_type.get_quads() + i % block_type.get_quad_number(), sizeof(Quad));
