...
def get_ao(local_pos, world_pos, world_voxels, plane):
    ...
    if plane == 'Y':
        ...
        d = is_void((x - 1, y, z + 1), (wx - 1, y, wz + 1), world_voxels)
...
def build_chunk_mesh(chunk_voxels, format_size, chunk_pos, world_voxels):
    vertex_data = np.empty(CHUNK_VOL * 18 * 5 * format_size, dtype='uint8')
    index = 0
        ...
        for y in range(CHUNK_SIZE):
            for z in range(CHUNK_SIZE):
                ...
                if is_void((x, y + 1, z), (wx, wy + 1, wz), world_voxels):
                    ...
                    v1 = to_uint8(x + 1, y + 1, z    , voxel_id, 0, ao[0])
                    v2 = to_uint8(x + 1, y + 1, z + 1, voxel_id, 0, ao[1])
                    v0 = to_uint8(x    , y + 1, z    , voxel_id, 0, ao[2])
                ...
                # bottom face
                if is_void((x, y - 1, z), (wx, wy - 1, wz), world_voxels):
                    ao = get_ao((x, y - 1, z), (wx, wy - 1, wz), world_voxels)
                ...
                if is_void((x + 1, y, z), (wx + 1, wy, wz), world_voxels):
                    ao = get_ao((x + 1, y, z), (wx + 1, wy, wz), world_voxels)
                ...
                if is_void((x, y, z - 1), (wx, wy, wz - 1), world_voxels):
                    ao = get_ao((x, y, z - 1), (wx, wy, wz - 1), world_voxels)
